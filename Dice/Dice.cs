...
namespace HeroSystemEngine.Dice 
{
    ...
    public class NormalDamageDicePool : DicePool, DamageDicePool
    {
        ...
        public DamageAmount DamageResult
        {
            get
            {
                DamageAmount theDamage = new DamageAmount();
                for (int i = 0; i < Pool.Length; i++)
                {
                    theDamage.STUN += ((NormalDamageDice)Pool[i]).Stun;
                    theDamage.BOD += ((NormalDamageDice)Pool[i]).Body;
                }
    ...
    public class KillingDamageDicePool : DicePool, DamageDicePool
    {
        ...
        public DamageAmount DamageResult
        {
            get
            {
                ...
                for (int i = 0; i < Pool.Length; i++)
                {
                    theDamage.BOD += ((KillingDamageDice)Pool[i]).Body;
                }
                theDamage.STUN = theDamage.BOD * Multiplier;
